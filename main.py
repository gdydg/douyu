...
class DouYu:
    ...
    def get_real_url(self):
        ...
        if error == 0:
            ...
            key = self.get_js()
        real_url = {}
        # 这里默认返回 flv，如果你想用 m3u8，可以参考下面这行注释掉的代码
        # real_url["m3u8"] = "http://hls3-akm.douyucdn.cn/live/{}.m3u8".format(key)
...
if __name__ == '__main__':
    ...
    print("开始读取 rooms.txt 配置文件...")
    
    try:
        # 打开同目录下的 rooms.txt 文件
        with open('rooms.txt', 'r', encoding='utf-8') as f:
            for line in f:
                # 忽略空行和没有冒号的行
                if ':' in line:
                    # 分割房间号和名称
                    rid, name = line.strip().split(':', 1)
                    print(f"正在抓取: {name} (房间号: {rid})")
                    
                    try:
                        spider = DouYu(rid)
                        urls = spider.get_real_url()
                        # 获取 FLV 直播流地址
                        real_url = urls.get("flv", "")
                        
                        if real_url:
                            # 拼接到 M3U 内容中
                            m3u_content += f"#EXTINF:-1 tvg-name=\"{name}\" group-title=\"斗鱼直播\",{name}\n"
                            m3u_content += f"{real_url}\n"
                            print(f"✅ {name} 获取成功")
        ...
        print("错误：找不到 rooms.txt 文件！")
        print("请在 main.py 同级目录下创建一个 rooms.txt，格式如：\n9999:YYF\n71415:大司马")
        exit(1)

    # 将最终结果写入 douyu.m3u 文件
    with open('douyu.m3u', 'w', encoding='utf-8') as f:
        f.write(m3u_content)
        
    print("\n🎉 任务完成！所有直播源已保存到 douyu.m3u")
